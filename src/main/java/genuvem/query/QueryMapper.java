...
import genuvem.index.IndexBuilderMapper;
import genuvem.io.IndexEntryWritable;

public class QueryMapper extends Mapper<Text, PartialSequence, Text, IndexEntryWritable> {
	...
	protected void map(Text key, PartialSequence value, Context context) throws IOException, InterruptedException {
		...
		logger.info("Cleaning up line breaks in query file " + header + ".");
		String seq = value.getValue().replace("\n", "");

		int currentIndex = 0;

		logger.info("Starting the split of the query file " + header + ".");
